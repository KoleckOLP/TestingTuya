...
for index, element in enumerate(body_content.find_all('p')):
    ...
    if index == 0:  # indoor temperatture
        in_temp = get_data(in_temp, in_temp_prev)
    elif index == 1:  # indoor humidity
        in_hum = get_data(in_hum, in_hum_prev)
    elif index == 2:  # indoor pressure
        in_press = get_data(in_press, in_press_prev)
    elif index == 3:  # indoor altitude (very inacurate)
        in_alt = get_data(in_alt, in_alt_prev)
    # Outdoor
    '''
...
#print(in_temp, in_hum, in_press, in_alt)
